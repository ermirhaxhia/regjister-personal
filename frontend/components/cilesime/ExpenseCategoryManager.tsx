...
function countLabel(n: number): string {
  if (n <= 0) return "";
  return n === 1 ? "1 shpenzim" : `${n} shpenzime`;
}

function deleteMessage(c: ExpenseCategory): string {
  if (c.expense_count > 0) {
    const verb = c.expense_count === 1 ? "e ka" : "e kanë";
    return `«${c.name}» — ${countLabel(c.expense_count)} ${verb} këtë kategori. Ato mbeten, por kategoria s'do të jetë më në listë.`;
  }
  return `Do të fshihet kategoria «${c.name}».`;
}
...
function Row({
  ...
  onRenamed: (c: ExpenseCategory) => void;
  onAskDelete: (c: ExpenseCategory) => void;
}) {
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      const row = await updateExpenseCategory(item.id, next);
      skipBlur.current = true;
      onRenamed(row);
      setEditing(false);
    } catch (err) {
      setError(
        err instanceof ApiError
          ? err.status === 409
            ? "Ekziston një kategori me këtë emër."
            : err.message
          : "Ndryshimi dështoi.",
      );
    } finally {
...
export default function ExpenseCategoryManager({
  ...
  const apply = useCallback((list: ExpenseCategory[]) => setItems(list), []);
  const { status, reload } = useGenLoad(listExpenseCategories, apply);
  ...
  const add = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const row = await createExpenseCategory(next);
  ...
  const onRenamed = (c: ExpenseCategory) =>
  ...
  return (
    ...
    >
      <p className="text-xs text-text-lo">
        Kategoritë dalin te dropdown-i kur shton një shpenzim. Riemërtimi
        përditëson edhe shpenzimet ekzistuese.
      </p>
